...
FROM (
  SELECT
    ...
    issn,
    page,
    ARRAY_AGG(STRUCT(EXTRACT(DATE FROM TIMESTAMP_MILLIS(lic.date)) AS date,
                             lic.url,
                             lic.delay_in_days,
    ...
    indexed
  FROM
    `api-project-764811344545.cr_dump.cr_jan21`, UNNEST(license) AS lic, UNNEST(link) AS lin
  GROUP BY doi,
           title,
           issued,
           issued_year,
